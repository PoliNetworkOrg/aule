...
- Every view and control now renders in React with strict TypeScript. Typed controllers preserve the original gestures, animation calculations, native events, view transitions, and browser fallbacks. React's application effect starts initialization after the shell commits and releases controllers, listeners, observers, pending timers, and motion resources on unmount. `allowJs` is disabled, and no frontend JavaScript modules remain. The old dynamic startup bridge and imperative HTML renderers are removed.
- TanStack's Vite plugin generates the route tree from `src/routes/`. File routes own matching and parameter decoding, and React route effects apply the typed view transitions once application initialization completes. The generated route tree retains TanStack's own emitted type assertions; handwritten application modules have no `any` or TypeScript checking suppressions. The browser-history adapter preserves `#info`, numeric/named classroom bookmarks, the hash-free home URL, outer query strings, and browser back/forward, including native hash entries with null history state. Available/Campus tab selection remains outside the URL, as upstream.
...
`.oxlintrc.json` includes the upstream anti-slop config's `oxc/no-accumulating-spread` and every generic vendored rule documented in its README. The optional Effect rules are not applicable. `no-runtime-typeof` permits explicit type guards, avoiding a new validation dependency. Unused state was removed, statement ternaries became `if` statements, and forced layout reads are explicitly `void`.

`@shadcn/lint` runs inside the `lint` script's Oxlint invocation. `shadcn/no-unknown-classes` and `shadcn/no-raw-colors` are enabled. The original external icon classes and named styling/state markers are allowed explicitly. These rules check the React view and control markup. The allowlist includes source state markers and classes declared in shadow stylesheets or the critical splash CSS in `index.html`. No ESLint or Prettier pipeline/configuration was introduced. Oxfmt checks formatting after lint.
...
The counts and remaining work below describe each commit at that point in the migration.
...
## Completed React/TypeScript migration

React owns all application views and controls, including startup readiness and route activation. The application effect releases layout observers, global events, deferred startup work, page controllers, and blur benchmarking. Shell translations and splash errors render in React. The remaining time-picker backdrop is a React portal; its pending frames, transitions, and motion callbacks are cancelled on teardown. Page-controller generations prevent old transition callbacks from touching a remounted app. Campus controllers reset their mounting state and retain typed references to each Mapbox instance.

The frontend contains no `.js`/`.jsx` modules or imperative HTML renderers, and `allowJs` is false. The old `loadApplication`/`PageController` bridge is removed. No new dependencies, committed tests, or intentional product changes were introduced. The critical static splash still appears before React loads.

Final production/beta builds, strict application TypeScript, full lint/format checks, and both worker typechecks pass. Beta favicons and the API preconnect match their source assets; built CSS retains both blur declarations. Temporary browser checks against the pinned source passed for desktop/mobile availability and overview navigation, classroom details and lesson popovers, search/favourites, Info and GitHub cache recovery, mobile time entry, locale failure/retry, partial occupancy failures and reloads, live Mapbox markers/recentering, numeric/named bookmarks, native hash changes, outer queries, and back/forward navigation. The source occasionally reports skipped view transitions or blocked haptics; the migrated comparisons reported no JavaScript exceptions.
