...
function buildPopup() {
  ...
  const {
    sectionEl: campusSectionEl,
    retranslate: retranslateCampus,
    refreshIfNeeded,
  } = buildCampusSection();
  ...
  const intervalStepper = buildStepper(
    savedHours,
    1,
    12,
    (v) => `${v}h`,
  ...
  const showPartialToggle = buildToggle(
    showPartialSaved === null ? true : showPartialSaved === "true",
  );
  ...
  autoSearchWarning.className =
    "settings-warning" + (autoSearchOn ? "" : " settings-warning--hidden");
  ...
  liveSearchWarning.className =
    "settings-warning" + (liveSearchOn ? "" : " settings-warning--hidden");
