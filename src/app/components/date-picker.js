...
export function setupDatePicker(getPreferInitialDate = () => null) {
  ...
  const DAY_NAMES = Array.from(
    { length: 7 },
    (_, i) => dayFormatter.format(new Date(2000, 0, 2 + i)), // Jan 2 2000 = Sunday
  );
  ...
  allDates.forEach((dateStr, index) => {
    ...
    if (!isSkipped) {
      datePicker.insertAdjacentHTML("beforeend", `<option value="${dateStr}">${dateStr}</option>`);
    }
  ...
  document.fonts.ready.then(() => {
    requestAnimationFrame(() => {
      ...
      const preferred =
        preferInitialDate &&
        [...elements].find(
          (el) => el.dataset.date === preferInitialDate && !el.classList.contains("date-skipped"),
        );

      const firstAvailable = [...elements].find((el) => !el.classList.contains("date-skipped"));

      if (preferred || firstAvailable)
        pillSelector.selectElement(preferred || firstAvailable, { animate: false });
