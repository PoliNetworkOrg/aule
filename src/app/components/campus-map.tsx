...
function updateShifted() {
  ...
  const centered =
    Math.abs(px.x - targetX) <= CENTER_SLACK_PX && Math.abs(px.y - targetY) <= CENTER_SLACK_PX;
...
function flyOpts(extra: CameraOptions, mobileHeightOverride?: number) {
  return {
    ...
    essential: true,
    padding: mapPadding(mobileHeightOverride),
    ...extra,
  };
}
...
function mapPadding(mobileHeightOverride?: number) {
  ...
  const bottom = Math.min(
    height + SHEET_MOBILE_GAP,
    Math.max(0, innerHeight - SHEET_MOBILE_MIN_VISIBLE),
  );
...
function flyToBuilding(_mapboxgl: MapboxLibrary, building: Building) {
  ...
  startFly(
    { center: [building.long, building.lat], zoom: BUILDING_FLY_ZOOM, pitch: 55, bearing: 0 },
    heightAfterBuildingSelect(),
  );
}
...
function showCampusMarkers(mapboxgl: MapboxLibrary) {
  ...
  for (const campus of campuses()) {
    ...
    if (!isNumber(lat) || !isNumber(long)) continue;
