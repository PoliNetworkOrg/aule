...
// the search overlay, which morphs out of it (see search-overlay-controller.ts).
//
...
const icon = (name: string) => `<i class="hgi-stroke ${name}" aria-hidden="true"></i>`;

// A tab's id is the id of the panel it shows.
const TABS = [
  {
    id: "available-classrooms-container",
    labelKey: "tabs.available",
    icon: icon("hgi-calendar-03"),
  },
...
type TabBar = ReturnType<typeof createTabBar>;

let mounted: TabBar | null = null;

/* --- Programmatic tab activation (e.g. the building header's jump button) - */
export function activateGroupTab(target: string) {
  mounted?.select(target, { silent: false });
}

// The labels are built when the bar mounts, before the locale is known, so this
// runs once i18n is ready and on every language switch.
// (setLabel also updates the tab definitions, so a later layout change keeps them.)
function retranslateNav() {
  for (const { id, labelKey } of [...TABS, SEARCH]) mounted?.setLabel(id, t(labelKey));
}

function initNavigation() {
  const root = document.getElementById("bn-wrapper")!;
  const contentContainers = document.querySelectorAll<HTMLElement>(".tab-content");
    ...
    contentContainers.forEach((container) => {
      if (container.id === targetId) {
        const id = requestAnimationFrame(() => {
          ...
          // styles. Let listeners (the date picker) recompute now that this
  ...
  const bar = createTabBar(root, {
    ...
    onSelect(id: string, { silent }: { silent: boolean }) {
      showContent(id);
  ...
  // page pads itself around it, and animates that padding: see style.css);
  // --bn-tabbar-height / --bn-tabbar-outer-inset: the pill's own thickness and
  // its clearance from the viewport edge, for the campus sheet's concentric
  // corners and campus-map.css.
  const barEl = root.querySelector<HTMLElement>(".lg-tabbar__bar");
  ...
  function setNavSizeVars() {
    ...
    // The campus sheet builds its corners around these, so it needs to know.
...
export function BottomNavigation() {
  const version = useSyncExternalStore(onTranslationChange, getTranslationVersion);

  useLayoutEffect(initNavigation, []);
  useLayoutEffect(retranslateNav, [version]);

  return <div className="bn-wrapper" id="bn-wrapper" />;
}
