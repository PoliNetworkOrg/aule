...
import { t, onTranslationChange, getTranslationVersion } from "../i18n.ts";
...
let activateMounted: ((target: string) => void) | null = null;

export function activateGroupTab(target: string) {
  activateMounted?.(target);
}
...
function initNavigation() {
  ...
  const stopRender = onSpringFrame(render);
  ...
  barItems.querySelectorAll<HTMLButtonElement>(".bn-tab-item").forEach((btn, i) => {
    btn.addEventListener(
      "click",
      () => {
        if (i !== groupIndex) animateGroupTap(i);
      },
      { signal: events.signal },
    );
  });
  searchBtn.addEventListener("click", animateSearchTap, { signal: events.signal });
  /* --- State + layout -------------------------------------------- */
  ...
  let anchors: { pos: number; size: number }[] = [];
  ...
  function measureAnchors() {
    ...
    return Array.from(barItems.querySelectorAll<HTMLElement>(".bn-tab-item")).map((el) => {
  ...
  function layout() {
    ...
    anchors.forEach((anchor, i) => {
      const el = activeRow.querySelectorAll<HTMLElement>(".bn-tab-active")[i];
  ...
  addEventListener("resize", layout, { signal: events.signal });
  ...
  function updateMask(scMain = scale.value, scCross = scale.value) {
    ...
    const cx =
      (vertical ? itemsW / 2 : pillPos.value + pillMain.value / 2) +
      (vertical ? crossOff.value : 0);

    const cy =
      (vertical ? pillPos.value + pillMain.value / 2 : itemsH / 2) +
      (vertical ? 0 : crossOff.value);

    const x = cx - w / 2;
    const y = cy - h / 2;

    const d =
      `M0 0H${itemsW}V${itemsH}H0Z ` +
      `M${x + r} ${y}H${x + w - r}A${r} ${r} 0 0 1 ${x + w} ${y + r}V${y + h - r}A${r} ${r} 0 0 1 ${x + w - r} ${y + h}H${x + r}A${r} ${r} 0 0 1 ${x} ${y + h - r}V${y + r}A${r} ${r} 0 0 1 ${x + r} ${y}Z`;

    const clip = `path(evenodd, "${d}")`;
    barItems.style.clipPath = clip;
    barItems.style.setProperty("-webkit-clip-path", clip);
  }
  ...
  function render() {
    ...
    const scaleStr = vertical ? `scale(${scCross}, ${scMain})` : `scale(${scMain}, ${scCross})`;
    ...
    const cMain = containerOff.value,
      cCross = containerCross.value;
  ...
  function animateGroupTap(i: number) {
    setGroupActive(i);
  ...
  function animateSearchTap() {
    ...
    later(() => searchScale.to(1, { stiffness: 350, damping: 30, mass: 0.8 }), 250);
  }
  ...
  let samples: { main: number; cross: number; t: number }[] = [];
  ...
  const mainDelta = (e: PointerEvent) => (isVertical() ? e.clientY - startY : e.clientX - startX);

  const crossDelta = (e: PointerEvent) => (isVertical() ? e.clientX - startX : e.clientY - startY);

  const mainPos = (e: PointerEvent) => (isVertical() ? e.clientY : e.clientX);

  const crossPos = (e: PointerEvent) => (isVertical() ? e.clientX : e.clientY);
  ...
  const pillEdgeAtPointer = (e: PointerEvent) => mainPos(e) - itemsOrigin - pillMain.value / 2;
  ...
  function onDragStart(e: PointerEvent) {
    ...
    if (absoluteDrag) holdTimer = later(() => engage(e), HOLD_MS);
  ...
  function release(e: PointerEvent, terminated: boolean) {
    ...
    if (dragTab) {
      ...
      barItems.addEventListener("click", swallow, {
        capture: true,
        once: true,
        signal: events.signal,
      });
      later(() => barItems.removeEventListener("click", swallow, { capture: true }), 0);
      dragTab = null;
    }
  ...
  function attachPillDrag(el: HTMLElement) {
    el.addEventListener("pointerdown", onDragStart, { signal: events.signal });
    el.addEventListener("pointermove", onDragMove, { signal: events.signal });
    el.addEventListener("pointerup", (e) => release(e, false), { signal: events.signal });
    el.addEventListener("pointercancel", (e) => release(e, true), { signal: events.signal });
  }

  attachPillDrag(pillHit);

  barItems.querySelectorAll<HTMLElement>(".bn-tab-item").forEach(attachPillDrag);
  ...
  function setNavSizeVars() {
    ...
    document.documentElement.style.setProperty(
      "--bn-tabbar-outer-inset",
      `${Number.isFinite(padLeft) ? padLeft : 28}px`,
    );
  }

  const sizeObserver = new ResizeObserver(setNavSizeVars);
  ...
  activateMounted = activate;

  return () => {
    activateMounted = null;
    events.abort();
    stopRender();
...
export function BottomNavigation() {
  ...
  useLayoutEffect(initNavigation, []);
    ...
    <div className="bn-wrapper" id="bn-wrapper">
      <div className="bn-group" id="bn-group">
        <nav className="bn-tabbar" id="bn-bar" aria-label="Main navigation">
          <div className="bn-tabbar-items" id="bn-bar-items">
            {GROUP_TABS.map((tab, i) => (
              <button
                key={tab.target}
                className="bn-tab-item"
                data-target={tab.target}
                role="tab"
                aria-selected={i === 0}
                aria-controls={tab.target}
              >
                <span className="bn-tab-content">
                  <i className={`hgi-stroke ${tab.icon}`} />
                  <span className="bn-tab-label" data-react-owned="" data-i18n={tab.labelKey}>
                    {t(tab.labelKey)}
                  </span>
                </span>
              </button>
        ...
        <div className="bn-pill-outer" id="bn-pill" aria-hidden="true">
          <div className="bn-pill-inner">
            <div className="bn-active-row" id="bn-active-row">
              {GROUP_TABS.map((tab) => (
                <span key={tab.target} className="bn-tab-content bn-tab-active">
                  <i className={`hgi-stroke ${tab.icon}`} />
                  <span className="bn-tab-label" data-react-owned="" data-i18n={tab.labelKey}>
                    {t(tab.labelKey)}
                  </span>
        ...
        <div className="bn-pill-hit" id="bn-pill-hit" />
      </div>
      <button
      ...
      >
        <span className="bn-search-btn-inner">
          ...
          <span className="bn-tab-label" data-react-owned="" data-i18n="tabs.search">
            {t("tabs.search")}
          </span>
        </span>
      </button>
