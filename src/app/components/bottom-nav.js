...
onLanguageSwitch(() => {
  bar.querySelectorAll(".bn-tab-label").forEach((el, i) => {
    el.textContent = tabLabel(GROUP_TABS[i]);
  });
  activeRow.querySelectorAll(".bn-tab-label").forEach((el, i) => {
    el.textContent = tabLabel(GROUP_TABS[i]);
  });
  searchBtn.querySelector(".bn-tab-label").textContent = t("tabs.search");
});
...
function updateMask(scMain = scale.value, scCross = scale.value) {
  ...
  const cx =
    (vertical ? itemsW / 2 : pillPos.value + pillMain.value / 2) + (vertical ? crossOff.value : 0);

  const cy =
    (vertical ? pillPos.value + pillMain.value / 2 : itemsH / 2) + (vertical ? 0 : crossOff.value);
