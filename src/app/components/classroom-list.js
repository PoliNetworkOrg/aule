...
async function _loadCardPhoto(classroomId, card) {
  ...
  img
    ...
    .catch(() => card.classList.add("photo-failed"));
}

const _photoObserver = new IntersectionObserver(
  (entries) => {
    for (const entry of entries) {
      if (!entry.isIntersecting) continue;
      _photoObserver.unobserve(entry.target);

      if (entry.target.dataset.idfoto)
        _loadCardPhoto(Number(entry.target.dataset.openClassroom), entry.target);
    }
  },
  { rootMargin: "300px" },
);
...
export function buildCardForClassroom(
  ...
  el.className = hasPhoto
    ? "classroom-card classroom-card--photo"
    : "classroom-card classroom-card--plain";
