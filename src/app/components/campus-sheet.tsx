...
function bounds() {
  ...
  const full = desktopMQ.matches ? available : Math.min(available * FULL_FRAC, available - 40);
...
function tabbarHeightPx() {
  const n = parseFloat(
    getComputedStyle(document.documentElement).getPropertyValue("--bn-tabbar-height"),
  );
...
function tabbarOuterInsetPx() {
  const n = parseFloat(
    getComputedStyle(document.documentElement).getPropertyValue("--bn-tabbar-outer-inset"),
  );
...
function renderSheet() {
  ...
  if (size.value !== lastDispatchedHeight) {
    ...
    document.dispatchEvent(
      new CustomEvent("campussheetresize", { detail: { height: size.value } }),
    );
  }
...
function onPointerMove(e: PointerEvent) {
  ...
  if (dragMode === "resize") {
    ...
    size.set(
      raw < min ? min + rubber(raw - min, GIVE) : raw > max ? max + rubber(raw - max, GIVE) : raw,
    );
  } else {
    const dy = y - dragStartY;
  ...
  if (mode === "resize") {
    ...
    snapToDetent(
      Math.abs(v) > FLING_VELOCITY
        ? flungDetentKey(size.value, v < 0 ? 1 : -1, Math.abs(v))
        : nearestDetentKey(size.value),
    );
  } else if (mode === "scroll") {
    ...
    scrollPos.v = -v * 1000;
    scrollPos.to(
      Math.min(max, Math.max(0, scrollPos.value - v * SCROLL_PROJECTION)),
      SETTLE_SPRING,
    );
  }
...
function onWheel(e: WheelEvent) {
  ...
  if (wheelMode === null) {
    wheelMode =
      detent !== "full"
        ? "resize"
        : (!contentScrollable() || scrollPos.value <= 0) && e.deltaY < 0
          ? "resize"
          : "scroll";
    wheelCommitted = false;
    wheelLastT = null;
  ...
  if (wheelMode === "resize") {
    ...
    if (dt != null && Math.abs(wheelVel) > FLING_VELOCITY) {
      ...
      if (next > max && contentScrollable()) {
        ...
        size.set(
          next < min
            ? min + rubber(next - min, GIVE)
            : next > max
              ? max + rubber(next - max, GIVE)
              : next,
        );
      }
    }
