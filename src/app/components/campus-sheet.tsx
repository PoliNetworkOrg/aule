...
//   - its footprint, exported for campus-map.tsx, which keeps its own padding
//     (and, while a building or campus is focused, the camera) glued to the
//     sheet through the `campussheetresize` event;
//   - opening a collapsed sheet when a building is selected.
//
// Lives inside #search-classrooms-container, alongside the map (see
// campus-map.tsx) and above it, below the header / footer / bottom-nav chrome.
...
const SQUIRCLE_RADIUS_SCALE =
  typeof CSS !== "undefined" && CSS.supports("corner-shape", "superellipse") ? 1.15 : 1;
...
const rootPx = (name: string, fallback: number) => {
  const n = parseFloat(getComputedStyle(document.documentElement).getPropertyValue(name));

  return Number.isFinite(n) && n > 0 ? n : fallback;
};

// Read/write the sheet's own scroll offset: used by campus-buildings.tsx to keep
...
export function getContentScroll(): number {
...
// bottom sheet, full width) driving campus-map.tsx's own auto-alignment
// padding. Safe to call before initCampusSheet() ever runs.
export function getSheetHeightPx(): number {
...
// triggering that expansion. campus-map.tsx's building fly-to reads this
// instead of the sheet's current (about-to-be-stale) height, so the camera
// aims at where the sheet will end up.
export function heightAfterBuildingSelect(): number {
...
// settle spring just animating on its own afterwards. campus-map.tsx's fly-to
// reads this to tell "the sheet is auto-expanding on its own" apart from "the
// user is dragging it somewhere else mid-flight".
export function isUserResizing(): boolean {
  return !!sheet?.isGesturing;
}

function detentAfterBuildingSelect(): string {
...
// bar has settled (see attachSheet).
...
// content.
function build(detent: string) {
  const desktop = desktopMQ.matches;

  builtGeometryKey = geometryKey();

  const created: Sheet = createSheet({
    header,
    content,
    container,
    label: t("tabs.campus"),
    ...
    detent,
    margin: {
      top: desktop ? headerHeightPx() + 32 + 20 : 0,
      bottom: 20,
      inline: desktop ? 20 : PLAIN_INSET,
    },
    ...(desktop ? { width: 420, side: "end" } : { geometry: mobileGeometry() }),
    onResize(height: number) {
      // campus-map.tsx listens for this: any manual drag/wheel, the settle
      // spring that follows one, or the auto-expand a building selection
      // triggers, alike.
      document.dispatchEvent(new CustomEvent("campussheetresize", { detail: { height } }));
    },
    onDetentChange(id: string) {
      created.el.dataset.detent = id;
    },
  });

  sheet = created;
  created.el.classList.add("campus-sheet");
  created.el.dataset.detent = created.detent;
  created.contentEl.classList.add("campus-sheet-content");
}
...
function attachSheet() {
  ...
  desktopMQ.addEventListener("change", rebuild, { signal: events.signal });
  ...
  // campus-buildings.tsx dispatches this whenever a building becomes selected
  // (from its grid, or a map marker tap): surface the sheet if it's currently
  // just a collapsed peek. A no-op at 'half'/'full': there's no reason to
  // shrink an already-open sheet just because a different building was picked.
  document.addEventListener(
    "buildingpageopen",
    () => {
...
export function CampusSheet() {
  ...
  return mounted ? <SheetController /> : null;
}
