...
class ClassroomDetail {
  constructor() {
    ...
    this._pendingTrigger = null; // { cardEl } stored by click handler before the route changes
  ...
  init(staticData) {
    ...
    const clearPressed = () => {
      document
        .querySelectorAll(".classroom-card--pressed")
  ...
  openRoute(id, campus, name, initial) {
    this._buildFlatIndex();
      ...
      if (!container) {
        ...
        if (header) {
          header.insertAdjacentHTML(
            "beforebegin",
            '<div class="detail-photo-container"><img class="detail-photo" alt=""></div>',
          );
    ...
    try {
      ...
      const ticksHtml = (() => {
        ...
        for (let m = DAY_START + 60; m < DAY_END; m += 60) {
          ...
          ticks.push(
            `<div class="detail-schedule-tick" style="--pos:${left}%"><span>${minutesToTimeDisplay(m)}</span></div>`,
          );
        }
      ...
      container.addEventListener("mousemove", (e) => {
        ...
        if (_activeBar && _activeBar !== bar) {
          const prevCursor = _activeBar
            .closest(".detail-schedule-bar-wrapper")
            ?.querySelector(".timeline-hover-cursor");
      ...
      const timelinePopoverBody =
        timelinePopoverEl?.querySelector(".timeline-popover-body") ?? null;
